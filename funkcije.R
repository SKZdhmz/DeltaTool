...
delta_mjerenja<-function(godina,postaja=NULL,data_id=c("NO2","NOx","O3","PM10","PM2.5","SO2","NO"),fpath="F:\\Data/MJERENJA/",delta_pp="postaje_delta.xlsx",verzija="16") {
      ...
      data_all<-gsub(x=gsub(x = dir(path =fpath,all.files = T,recursive = T,pattern = paste0(".*",verzija,".xlsx")),pattern = "svi podaci ",replacement = ""),pattern = " [0-9][0-9].*",replacement = "")
      ...
      pattern<-paste(".*",postaja,".*",verzija,".xlsx",sep = "")
